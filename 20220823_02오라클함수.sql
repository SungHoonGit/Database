...
D:\java202207\database\20220823_02오라클함수.sql
////////////////////////////////////////////////////////////////////////////////
...
select rpad('Sky',5, '*') from dual;                --5칸 내에서 출력하고 오른쪽 빈칸은 *로채움
select replace('Hello World','W', 'p') from dual;   --특정문자 변환
...
--문자열 연결하기
select concat('로미오','줄리엣') from dual; --로미오 줄리엣
--concat(칼럼명, '문자열')
select concat('uname', '의 평균은'), concat('aver', '입니다.') from dual;
...
select round(avg(kor),2) from sungjuk;  -- 66.36    반올림함수
...
-- 날짜데이터의 연산
select sysdate+100 from dual;       -- 오늘날짜 + 100
select sysdate-100 from dual;       -- 오늘날짜 - 100
