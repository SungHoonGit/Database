...
D:\java202207\database\20220822_04제약조건.sql
////////////////////////////////////////////////////////////////////////////////


★ [ 자료형 ]
1) 문자형
    - 가변형 varchar(5)   'SKY'   5칸이여도 3칸만 데이터 사용 
        ex) 아이디, 비밀번호, 이름, 주소 ~~
    - 고정형 char(5)      'SKY  ' 5칸.  
        ex) 주민번호, 우편번호, 계좌번호 [정해진 칸수] 
...
1) primary key
    기본키. 유일성
    where 조건절에 걸릴 수 있는 대표적인 컬럼을 강제할 수 있다.
    예) 주민번호, 핸드폰번호, 계좌번호, 아이디, 이메일 ~~~
    중복을 허용하지 않음.
    null값 허용하지 않음. (빈값을 허용하지 않음. 반드시 데이터 입력해야 함.)
...
4) default
    사용자가 값을 입력하지 않으면 해당 칼럼이 정이한 기본값을 자동으로 입력해준다.
    
5) unique
    중복된 데이터 들어오면 unique 로 출력!
    null를 한번만 허용! 
    
6) foreign key
    외래키
    테이브 조인하는 경우 부모와 자식 관계를 설정
...
-- primary key  제약조건
-- ORA-00001: 무결성 제약 조건(SYSTEM.SYS_C007327)에 위배됩니다
-- SNO 칼럼은 기본키(PK)이므로 중복된 값을 허용하지 않음.
insert into sungjuk(sno,uname) values(1, '홀길동'); -- 2번 실행했을 때 에러 발생 
